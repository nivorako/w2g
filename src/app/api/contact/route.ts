 ...
 *
 * @param {NextRequest} req - La requête HTTP
 * @returns {Promise<NextResponse>} La réponse HTTP
 *
 * @throws {Error} Si l'email ou le message est manquant, ou si le serveur rencontre une erreur inattendue.
 */
    ...
    try {
        ...
        if (!user || !pass || !to) {
            return NextResponse.json(
                {
                    error: "Configuration manquante. Veuillez définir GMAIL_USER, GMAIL_APP_PASSWORD et CONTACT_TO dans les variables d'environnement.",
                },
                { status: 500 },
            );
        }
